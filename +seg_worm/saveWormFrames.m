...
%   FAILEDFRAMES = seg_worm.saveWormFrames(wormFile, videoFile,*frames,*blockSize,*samples,*infofile,*log_file,*diff_file)
%
%
%   NOTE: This is very similar to segWormFrames ... :/
%
%
%
...
%   samples   : pass in to normalize the worms, the samples per normalized worm; 
%               if empty, the worms are in structs
%
%
%
%
    ...
    if isSeg
        ...
        if ~isempty(worm)
            ...
            if isempty(prevWorm)
                ...
                [worm, ~, ~] = orientWormAtCentroid(prevWorm, worm, orientationSamples);
            end
    ...
    if blockI == blockSize || i == length(frames) || isDone
        ...
        % Normalize the worms.
            ...
            [vulvaContours, nonVulvaContours, skeletons, angles ...
                ...
                vulvaAreas, nonVulvaAreas] = seg_worm.w.util.normWorms(block, samples, ...
        ...
        if isSaved
            save(worm_file_path, blockName, '-append');
        else
            save(worm_file_path, blockName);
            isSaved = true;
        end
        framesStatus(1:blockSize) = 'd';
        block = cell(blockSize,1);
...
% Note: I don't trust IEEE floating point arithmetic.
firstFrame = frames(1); %#ok<NASGU>
lastFrame  = frames(end); %#ok<NASGU>
blocks     = floor((i - 2) / blockSize) + 1; %#ok<NASGU>
if isSaved
    save(worm_file_path, 'blocks', 'blockSize', 'firstFrame', ...
