...
elseif strcmp(wildpath,'**')
  ...
  D = seg_worm.fex.rdir([prepath postpath(2:end)]);
  ...
  Dt = dir(''); 
  for ii = 1:length(tmp),
    Dt = [Dt; seg_worm.fex.rdir([prepath tmp(ii).name filesep wildpath postpath])]; %#ok<AGROW>
  end
  D = [D; Dt];
  ...
  D = dir(''); 
  for ii = 1:length(tmp),
    D = [D; seg_worm.fex.rdir([prepath tmp(ii).name postpath])]; %#ok<AGROW>
  end
