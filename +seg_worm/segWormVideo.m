function failedFrames = segWormVideo(videoFile, anglesVideoFile, touchVideoFile, debugVideoFile, varargin)
%SEGWORMVIDEO Segment a worm video.
%
%   - Segment the video:
%   FAILEDFRAMES = SEGWORMVIDEO(VIDEOFILE, ANGLESVIDEOFILE, TOUCHVIDEOFILE, DEBUGVIDEOFILE)
%
%   - Segment the video and normalize the worm:
%   FAILEDFRAMES = SEGWORMVIDEO(VIDEOFILE, ANGLESVIDEOFILE, TOUCHVIDEOFILE, DEBUGVIDEOFILE, SAMPLES)
%
%   - Segment the video, normalize the worm, then interpolate the worm:
%   FAILEDFRAMES = SEGWORMVIDEO(VIDEOFILE, ANGLESVIDEOFILE, TOUCHVIDEOFILE, DEBUGVIDEOFILE, SAMPLES, ISINTERP)
%
%   - Segment the video, normalize the worm, then interpolate the worm, and
%     exclude stage movements:
%   FAILEDFRAMES = SEGWORMVIDEO(VIDEOFILE, ANGLESVIDEOFILE, TOUCHVIDEOFILE, DEBUGVIDEOFILE, SAMPLES, ISINTERP, INFOFILE, LOGFILE, DIFFFILE)
%
%   Inputs:
...
if ~isempty(touchVideoFile)
    ...
    % Construct the video file.
    if ispc()
        tvw = videoWriter(touchVideoFile, 'fps', fps, 'plugin', 'DirectShow');
    else
        tvw = videoWriter(touchVideoFile, 'fps', fps, 'plugin', 'ffmpegDirect');
    end
end
...
if ~isempty(debugVideoFile)
    ...
    % Construct the video file.
    if ispc()
        dvw = videoWriter(debugVideoFile, 'plugin', 'DirectShow', 'fps', fps, 'fourcc', 'FFDS');
    else
        dvw = videoWriter(debugVideoFile, 'plugin', 'ffmpegDirect','fps', fps, 'fourcc', 'FFDS');
    end
end
