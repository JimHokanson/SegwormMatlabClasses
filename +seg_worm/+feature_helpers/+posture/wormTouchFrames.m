...
%Whenever a new start is found, find the first segmented frame, that's the
%end ...
...
for iFrame = 1:n_frames
    if in_coil 
        ...
        %Some mental math notes
        %--------------------------
        %start 5
        %good seg 7
        %# coil = 2 frames, frames 5 & 6
        %  7 - 5 = 2
        %-------------------------------------
        ...
        if n_coil_frames >= COIL_FRAME_THRESHOLD
           ...
           touchFrames(n_coils).start = coil_frame_start - 1;
           touchFrames(n_coils).end   = iFrame - 2; %:/
        end
        in_coil = false;
        end
